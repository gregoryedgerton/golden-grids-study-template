 ...
 */
type Key = "inspect" | "notes";
const STORAGE = "gg-tools";

function readInitial(): Record<Key, boolean> {
  const state: Record<Key, boolean> = { inspect: false, notes: false };
  if (typeof window === "undefined") return state;
  try {
    const saved = JSON.parse(localStorage.getItem(STORAGE) || "{}");
    for (const k of ["inspect", "notes"] as Key[]) if (typeof saved[k] === "boolean") state[k] = saved[k];
  } catch {}
  const q = new URLSearchParams(window.location.search);
  for (const k of ["inspect", "notes"] as Key[]) {
